...
namespace pharmacy.DAL
{
    public class DbHelper : IDisposable
    {
        ...
        {
             OpenConnection();
            var c = new SqlCommand("UPDATE Users SET failed_attempts=@f,last_attempt=GETDATE() WHERE username=@u", connection);
        ...
        {
            OpenConnection();
            var c = new SqlCommand("UPDATE Users SET failed_attempts=0,last_attempt=GETDATE() WHERE username=@u", connection);
        ...
        public static void f(string value, int id)
        {
            string role = "";
        ...
        public static int getid(string n)
        {
            ...
            string query = "SELECT id FROM Users WHERE username = @username AND IsDeleted = 0";
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                command.Parameters.AddWithValue("@username", n);
